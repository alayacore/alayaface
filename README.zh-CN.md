...
- **模型编辑器**呈现 AlayaCore 认识的每一个 `model.conf` 字段：`name`、`protocol_type`、`base_url`、`api_key`、`model_name`、`context_limit`、`max_tokens`、`reasoning_field`、按推理档位划分的 `reasoning_0/1/2` provider JSON，以及 `serial_tool_calls`。原因是 `model_sync` 会*整体替换*模型列表：编辑器没有携带的字段，在下一次保存时就会从 `model.conf` 中被删除。比当前构建更新的键会被原样保留而不是丢弃（见 `src-elm/src/Session/ModelConfig.elm`，由 `make check-schema` 校验）。
...
每个会话、每个计划都是同一张无限画布上的一个窗口：拖动标题栏移动窗口，拖动边缘
调整大小，在空白背景上右键打开全局菜单，在背景上滚轮缩放（整块画布一起缩放，窗口
随之重排）。

窗口里的内容永远撑满窗口：消息、消息块和输入框都随你把它拉多宽就变多宽，没有一条
"阅读栏宽度"的上限会让文字停在旧宽度、只剩外框在变宽——要多宽由你缩放决定。消息块
只有上下两条分隔线，左右不再有边框；左右内边距也随着边框一起去掉了——文字就贴着那条
线的端点排布，只保留窗口自身与边框之间的那点间距。

### 窗口叫什么名字

会话窗口的标题是它的**名字**：`<名字> — <模型>`。这个名字归你。你在一个会话里输入的
第一句话就成了它；想改成别的，在会话管理器里点 **✎ 重命名** 即可。名字属于会话本身，
不属于某个窗口、也不属于它的 fork，所以重跑一次、重启一次都还在——而只要你确认过一次
编辑器，之后就绝不会再有自动名字盖掉它，哪怕你填回的就是它自己猜出来的那几个字。

一个你既没命名、也没发过消息的会话，显示的还是从前那样：标题栏里 `Session <n>`，列表里
8 位 id。那个编号只是当前这一页给它排的位置，这正是列表宁可用它前缀 id 的原因，也是给
会话起个名字值得你敲一次键的原因。计划里的节点会话不命名——它们的 `[Plan · planId/nodeId]`
标记已经说明了身份。

会话管理器按名字过滤，命中的结果按名字排序；它同时也匹配 id，所以没命名的会话你照样找得
回来。名字在存储上限 120 个字符，在屏幕上按宽度截断，完整名字和 id 都在 tooltip 里：一个
你读不全的名字，不等于一个你弄丢了的名字。

**Solo 视图**是"一次只看一个窗口"的展示模式：该窗口铺满整个视口，其余窗口不再绘制。

| | |
|---|---|
| 进入 | 窗口标题栏的 `⤢`（会话与计划都有）、全局菜单的 **Solo 窗口（Solo window）**，或 `Ctrl+Shift+F`（作用于你当前正在看的窗口） |
| 退出 | **只能用鼠标**：同一个按钮（`⤡`），或全局菜单的 **退出 Solo（Exit solo）** |
| 会停下的操作 | 平移、拖动窗口、调整大小、滚轮缩放、计划信息窗——面板就是视口，这些手势已无对象可作用 |
| **不会**停下的操作 | 其他一切。流式输出、运行中的任务、工具确认、MCP 授权、计划执行、版本冻结全部照常继续：Solo 只是隐藏窗口，从不关闭窗口 |

窗口处于 Solo 时，标题栏会显示身后发生了什么——`画布 · 2 运行中 · 1 等待你`。这些数字
统计的是*被隐藏*的窗口：`等待你` 指某个窗口正弹着需要回答的界面（工具确认、关闭或取消
确认、文件选择器、模型选择器、MCP 初始化/授权、媒体预览），并会高亮显示——一个卡在
看不见的对话框上的会话，否则就会无声地停住；`运行中` 统计仍有任务在跑的隐藏窗口。点击
该控件回到画布，那个待处理的界面就在那里。

**任何键盘组合键都不会退出 Solo** —— 这是刻意的。键盘是打字和条件反射发生的地方，
而以前会退出 Solo 的两个键都是迟早会出事的手势：`Ctrl+W`（从浏览器借来的习惯）以前会
关闭最上层窗口（活动计划优先，否则是活动会话），所以它能一下把一个会话从画布上拿走；
`Esc` 则是万能的「先出去再说」。现在 `Ctrl+W` 完全不起作用，`Esc` 只会关掉当前打开的
浮层，然后就停下。`Ctrl+Shift+F` 仍然可以**进入** Solo，再按一次不会做任何事 ——
进入是一个请求，离开是一个按钮。

所以关闭窗口只属于每个窗口的 `✕`（可见、逐个窗口、并且永远先问一句：关闭会把对话留在
磁盘上、可随时恢复；关闭并删除才会删文件），离开 Solo 只属于 `⤡` 按钮或菜单项。而
Solo 下根本没有 `✕`：标题栏只剩这两个控件——在整块画布都看不见的时候关掉眼前这个会话，
正是这个视图唯一不能提供的误操作。退出 Solo 后，`✕` 就回到原来的位置。窗口仍然可能在
Solo 下消失（在会话管理器里删除、或按归属关系级联关闭），此时视图会自己退出，所以它
不可能指向一个已经不存在的窗口。

Solo 本身不保存任何东西：它返回的布局就是离开时的布局，因为进入 Solo 不改动任何 rect。
它被记住靠的是那块「画布存档」——`ui.conf` 里存了哪个窗口处于 Solo；由于会话不会自动重开，
这个 key 是以一条**待生效意图**落地的，等那个窗口再次被创建时才附着上去（见
`docs/solo-view.md`）。所以重启后先看到画布，等你重新打开那个会话，它才按你离开时的样子
全屏 Solo。退出 Solo 会清掉这条意图，于是三次重启前进入的 Solo 不会劫持一个用户从没要求
独处的会话。
