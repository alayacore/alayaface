...
| 退出 | 同一个按钮（`⤡`）、`Esc`、`Ctrl+W`，或全局菜单的 **退出 Solo（Exit solo）** |
...
`Ctrl+W` 不关闭任何窗口。它以前会关闭最上层那个窗口（活动计划优先，否则是活动的
会话），于是从浏览器借来的一个习惯动作就能让你丢掉一个会话；现在它在画布视图里完全
不起作用，在 Solo 里则返回画布——这是这个组合键还能做的唯一一件事，因为它不销毁任何
东西。关闭窗口只属于 `✕` 按钮：可见、逐个窗口、并且永远先问一句（关闭会把对话留在
磁盘上、可随时恢复；关闭并删除才会删文件）。用 `✕` 关闭处于 Solo 的窗口时，也会同时
退出 Solo，所以这个视图不可能指向一个已经不存在的窗口。
