...
- [ ] Focusing a session that belongs to a plan node: the curve to its node card is an **S-shape** (two reverse arcs); **no extra node↔node curves** are drawn — the DAG's own edges are the only node-to-node lines (P32)
- [ ] Plan window active → the plan↔session curve connects it to its owning session's `[Plan: …]` button (P27)
...
- [ ] **Cascade close (P34/P35)**: with a plan running, close the ORIGIN session (✕) → the plan window AND every node session window close, and the run does not respawn anything; closing the PLAN window (✕) → its node session windows close too (no respawn); closing a node session manually (its own window) still fails the node → retry (unchanged)
...
- [ ] **Connection chain (P36)**: focus a node's session → the plan window is raised to the second layer (session z = plan z + 1) and the **whole ancestor path** is drawn as solid, thicker (stroke-width 3) S-shaped bezier curves (two control points): the session→node curve PLUS every ancestor plan→owning-session curve up to the **top-level session window** (focus a DEEP node session → you can trace the lines all the way up); curves follow live when dragging/resizing/scrolling; closing the anchor session or focusing a plain session → curves disappear; node scrolled out of the canvas → that node curve hidden
