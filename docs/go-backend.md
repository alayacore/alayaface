...
| `resume_session` | `POST /rpc/resume_session` | `sessionId`, `binaryPath`, `workDir`(nullable), `planId`(nullable), `nodeId`(nullable), `originSessionId`(nullable), `clientId` | string sessionId (re-applies the persisted spawn args: tool-confirm policy, builtin-tools restriction, system prompt, reasoning level, work dir). Refused BEFORE spawning when the file records a different `message_version` than `SupportedMessageVersion` — nothing gets registered, so the answer is the reason every time instead of "Session is already active"; an unknown version is left to the core (plan-mode.md §8.8) |
