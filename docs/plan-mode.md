...
- **Node ↔ session connection curve (P19)**: when focusing a session that belongs to a plan node —
  ...
  - **no extra node↔node curves (P32, user clarification)**: the DAG canvas already draws the dependency edges between node cards, so focusing a node session draws ONLY the session→node curve — nothing is added between cards;
...
- **Plan window close cascades down too (P35)**: clicking ✕ on a plan window **stops its run** (if still InProgress/Paused) and **closes its node session windows** — same StopRun → CloseSessionFor path, so nothing respawns and sub-plans of those node sessions cascade. Terminal runs (Completed/FailedRun/Stopped) are left untouched: their node sessions are already closed and re-stopping would overwrite the status-bar status (e.g. Completed → Stopped);
...
| P35 | **plan-window close cascades down** (user request): closing a plan window (✕) stops its run (if InProgress/Paused — terminal runs untouched so the status bar keeps Completed/FailedRun/Stopped) and closes its node session windows (no respawn; their sub-plans cascade); `closeChildPlan` now delegates to `PlanClose`. plan-e2e gained step 8d | ✅ done |
