...
| C3 | **Elm pure logic** | Scheduling/validation/layout/state machine are all pure-function modules (`Plan/*`), fully covered by elm-test; side effects only go through `Ports.elm` ↔ `transport.js` |
...
- **Preset structure** (`~/.alayaface/presets/<name>/`) — **empty-shell seeds**:
  ...
  - `settings.conf` — **AlayaFace-owned, stored per preset**, `{"tool_confirm": "id1,id2", "builtin_tools": "id1,id2", "reasoning_level": 0|1|2, "system_prompt": "..."}`; not copied into session dirs; `get_global_settings(preset)` / `sync_global_settings(config, preset)` support per-preset read/write (sync has **merge semantics**: absent fields keep current values); both seed presets carry it (with the plan-mode system_prompt and `reasoning_level: 1`);
  - `themes/` — alayacore auto-creates default themes when missing.
- **create_session command**: `preset` is REQUIRED (there is no active preset); `toolConfirm` / `builtinTools` / `reasoningLevel` (0|1|2) default to the given preset's settings.conf; the system prompt (`--system`) defaults to the given preset's settings.conf `system_prompt` unless an explicit non-empty override is passed (the frontend sends only the recursion guard); **the reasoning level is passed to alayacore as `--reasoning-level=<n>`** (default 1) and persisted in `session.spawn.json` so resume re-applies it; **creating a session dir copies the given preset into `session_dir/config`** (excluding settings.conf).
...
- **Replay suppression (P24/P26)**: resuming a session replays its history,
...
- **P26 (ready SM)**: the marker is removed ONLY on the core's explicit
  readiness signal `SM {"type":"session","data":{"state":"ready"}}`
  (alayacore v0.62.4+, verified against the binary). Order on resume:
...
- **Connection chain (P36 — deep node sessions show their whole path)**: replacing the single P19 curve, focusing a session that belongs to a plan node (or activating a plan window) draws the **FULL ancestor chain** — every segment from that window up through each ancestor plan↔session pair to the **top-level session**, so the lines lead directly to the topmost session window:
  ...
  - **transport.js draws one bezier per segment** (a fixed SVG overlay per segment on `<body>`; rAF measures DOM `getBoundingClientRect` every frame; follows drag/resize/scroll). Node segments run from the midpoint of the session window edge nearest the node to the node card center, z-index = the plan window's value (above the plan, below the session); plan segments run from the plan window edge nearest the session to the session's **`[Plan: <planId>]` button** when visible (else the session edge), z-index = the top of the two participants;
  - **resumed sessions** resolve through P18's `planResumedFrom` (fresh id → original dir id) both for the focused session and for every ancestor origin (`liveSessionForOrigin`); node ids may contain `/`;
  - the chain **stops at the deepest drawable level** when an ancestor owning session/plan window is closed (transport.js hides segments whose participants are missing) and terminates safely on meta cycles;
  ...
  - both curve styles share the **solid, thicker style** (`stroke-width: 3`, no dasharray) and an **S-shaped cubic bezier** (`curvePath` in transport.js — two independent control points offset on **opposite sides** of the travel line, so the curve arcs one way then back across the midpoint; `bow = clamp(20, 80, dist × 0.18)`);
...
- Manually closing a NODE session window (while its plan is running) injects a disconnect event into the runner → the node fails and retries (unchanged); history stays on disk via run.json / session.alaya.
...
| New command | Rust | Go | transport.js | Ports.elm |
|---|---|---|---|---|
...
| `transport.js` + `Ports.elm` | `createSession` port carries `toolConfirm` / `preset` / `builtinTools` fields |
...
| P32 | user-driven UI/CI pass: **S-shaped bezier** (two control points on opposite sides) (§7.1); ~~ancestor-chain curves~~ **removed per user clarification** — the DAG already draws node↔node edges, so focusing a node session draws only the session→node curve (P19), and an active plan window connects only to its owning session's `[Plan: …]` button (P27); **overlay scrollbar** (native scrollbar hidden → messages stay exactly as wide as the prompt input; custom thumb floats over content, transport.js); **uniform Session Manager list buttons**; **no auto-created session at startup** (welcome screen, user opens sessions on demand) (§4); **GitHub CI extended** — Tauri `cargo build` + full E2E job (Go backend + fakecore + headless Chrome) | ✅ done |
| P33 | **page-refresh session reclaim** (user bug): a refresh orphans the backend's session handles (their windows died with the old page) → `resume_session` kept failing with "Session is already active" until the backend restarted. New `close_all_sessions` RPC (Rust+Go, graceful cancel-first close — history preserved) fired once on page load; restart-e2e gained a page-refresh phase | ✅ done |
