            ...
            , test "keyup with a missing PT session is a safe no-op" <|
                \_ ->
                    ...
                    Expect.all
                        ...
                        m1
            , test "a PT stop marks the in-flight transcription (ptTranscribing)" <|
                \_ ->
                    let
                        ( m1, _ ) =
                            App.Update.update (AT.PushToTalk True) initModelWithSession
                        ...
                        ( m3, _ ) =
                            App.Update.update (AT.PushToTalk False) m2
                    in
                    Expect.equal True m3.ptTranscribing
            , test "a plain mic-button stop never marks ptTranscribing" <|
                \_ ->
                    let
                        ( m1, _ ) =
                            App.Update.update AT.VoiceInput initModelWithSession

                        ( m2, _ ) =
                            App.Update.update AT.VoiceInput m1
                    in
                    Expect.all
                        [ \mm -> Expect.equal False mm.ptTranscribing
                        , \mm ->
                        ...
                        m2
            , test "a failed PT transcription clears ptTranscribing" <|
                \_ ->
                    let
                        ( m1, _ ) =
                            App.Update.update (AT.PushToTalk True) initModelWithSession
                            ...
                            App.Update.update
                                (AT.AsrResult
                                    (E.object
                                        ...
                                        , ( "ok", E.bool False )
                                        , ( "text", E.string "" )
                                        , ( "error", E.string "no mic" )
                                        ]
                                    )
                                )
                                m3
                    in
                    Expect.equal False m4.ptTranscribing
            , test "PT transcript insertion consumes ptTranscribing" <|
                \_ ->
                    let
                        ( m1, _ ) =
                            App.Update.update (AT.PushToTalk True) initModelWithSession
