...
//      locked) → stop → ONLY the WAV data URI is sent as a UA frame;
//      the typed text stays in the input box (not sent, not cleared);
//      fakecore echoes it back as an audio chip
//
...
try {
  ...
  // ── 8. raw audio → UA frame (audio only, input untouched) ────────
  // Type some text, record raw audio (input/send/ASR locked), stop →
  // ONLY the WAV data URI is sent as a UA frame; the typed text stays
  // in the input box (not sent, not cleared). fakecore echoes the UA
  // back and the session shows an audio chip.
  ...
  const typedValue = (await taState()).v; // capture before the send
  await page.$eval('.raw-btn', el => el.click());
